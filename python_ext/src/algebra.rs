...
use algebra::{AdemAlgebra as InnerAdem, Algebra, MilnorAlgebra as InnerMA};
...
/// The Adem basis of the (mod-`p`) Steenrod algebra.
#[pyclass(name = "AdemAlgebra", module = "sseq_ext")]
pub struct AdemAlgebra {
    pub inner: Arc<InnerAdem>,
    pub unstable: bool,
}

#[pymethods]
impl AdemAlgebra {
    /// `AdemAlgebra(p, unstable=False)`. The `unstable` flag enables the
    /// instability bookkeeping needed for unstable resolutions; leave it
    /// `False` for stable computations.
    #[new]
    #[pyo3(signature = (p, unstable=false))]
    fn new(p: u32, unstable: bool) -> PyResult<Self> {
        let p = ValidPrime::try_from(p)
            .map_err(|e| PyValueError::new_err(format!("Invalid prime: {e}")))?;
        Ok(Self {
            inner: Arc::new(InnerAdem::new(p, unstable)),
            unstable,
        })
    }
    ...
    fn __repr__(&self) -> String {
        if self.unstable {
            format!(
                "AdemAlgebra(p={}, unstable=True)",
                self.inner.prime().as_u32()
            )
        } else {
            format!("AdemAlgebra(p={})", self.inner.prime().as_u32())
        }
    }
}
