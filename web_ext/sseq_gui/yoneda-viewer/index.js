...
function drawModule(context, data, x, width, height, num_degree) {
    ...
    context.fillText(
        data.graded_dimension.reduce((a, b) => a + b, 0),
        transform.apply([mid, height])[0],
        height - BOTTOM_MARGIN * 0.7,
    );
    ...
    for (let i = 0; i <= num_degree; i++) {
        ...
        for (let j = 0; j < dim; j++) {
            context.beginPath();
            context.arc(
                ...transform.apply([mid + offset * (j - (dim - 1) / 2), y]),
                3,
                0,
                2 * Math.PI,
            );
    ...
    for (let action of data.actions) {
        ...
        const sourceX_ =
            mid + offset * (action.input_idx - (source_dim - 1) / 2);
        ...
        for (let j = 0; j < target_dim; j++) {
            if (action.output[j] != 0) {
                ...
                const [targetX, targetY] = transform.apply([
                    targetX_,
                    targetY_,
                ]);
                ...
                if (op == 1) {
                    ...
                    if (side[op] == 1) {
                        ...
                        context.arc(
                            sourceX - distance * Math.cos(angle - Math.PI / 3),
                        ...
                        let angle =
                            Math.PI - Math.acos((sourceX - targetX) / distance);
                        context.arc(
                            sourceX + distance * Math.cos(angle - Math.PI / 3),
                            sourceY - distance * Math.sin(angle - Math.PI / 3),
                            distance,
